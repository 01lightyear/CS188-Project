...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)#获得所有的conditional probabilities
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        for key in list(self.keys()):
            dict.__setitem__(self, key, self[key] / ratio)
        "*** END YOUR CODE HERE ***"
    ...
    def sample(self):
        ...
        rand = random.uniform(0, total_prob)#每一个value按其大小代表一段区间，当落在区间上时（即cumulative达到rand时即落在该区间上）
        cumulative = 0.0
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            prob = self.getObservationProb(observation, pacPosition, particle, jailPosition)
